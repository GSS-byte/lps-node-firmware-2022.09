...
static uint32_t twrAnchorOnEvent(dwDevice_t *dev, uwbEvent_t event)
{
  ...
  if (event == eventPacketReceived) {
    ...
    float rssi = dwGetReceivePower(dev);
    float fpPower = dwGetFirstPathPower(dev);
    float quality = dwGetReceiveQuality(dev);
    ...
    if (cfgIsBinaryMode()) {
      ...
      printf("Node %02X -> %02X | RSSI: %.2f dBm | FP: %.2f dBm | Quality: %.2f\r\n",
