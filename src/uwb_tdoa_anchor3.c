...
static void handleRxPacket(dwDevice_t *dev)
{
  ...
  debug("TDoA3 Rx: len %d, type %02x, src %02x\r\n", dataLength, rxPacket.payload[0], rxPacket.sourceAddress[0]);
...
static void setupTx(dwDevice_t *dev)
{
  ...
  debug("TDoA3 Tx: seq %d at %08x\r\n", ctx.seqNr, (unsigned int)ctx.txTime);
